...
Backlog còn lại (mỗi mục là một release tương lai): lazy schema cache sâu hơn, MongoDB/DuckDB/Oracle adapters, tunnel/proxy, cloud warehouse/lakehouse, object storage/file transfer, message queues, XLSX import/export, virtual grid, explain-plan, PNG graph export, procedure/trigger dependency graph, process/privilege manager, Marketplace publish. Xem trạng thái chi tiết bên dưới.
...
- 🔥 **MongoDB** adapter: collections, documents, indexes, JSON filter, explain cơ bản.
- 🔥 **DuckDB** adapter: query Parquet/CSV/JSON, analytics local, read-only mode an toàn cho file.
- 🔥 **Oracle** adapter: schema/table/view/procedure metadata, query runner, explain plan bước đầu.
...
- 🔥 **SSH tunnel** cho mọi server database: host key policy, local port allocation, reconnect và profile-level config.
- ⭐ **Socks Proxy / HTTP Proxy**: áp dụng cho driver hỗ trợ network proxy; fallback bằng tunnel helper khi driver không hỗ trợ trực tiếp.
- ⭐ **Docker connector**: phát hiện container database local, đọc port mapping, mở connection từ container đang chạy.
- ⭐ **JDBC bridge**: chạy qua sidecar/worker riêng để hỗ trợ engine hiếm mà Node driver không ổn định.
- ⭐ **S3** browser/query connector: duyệt bucket/prefix, preview CSV/JSON/Parquet, mở DuckDB query trên object.
- 💡 **FTP / WebDAV / RDP**: chỉ nên là connector phụ trợ/import/export, không đặt chung luồng SQL explorer.
- 💡 **Kafka / RabbitMQ** explorer: topics/queues, consumer preview, publish test message, schema registry nếu có.
...
- `v1.11.0` / Phase 22 — analytics/lakehouse: ClickHouse, Trino/Presto, Snowflake/BigQuery/Databricks/Athena nghiên cứu theo driver nhẹ.
- `v1.12.0` / Phase 23 — non-SQL connectors: S3, Kafka, RabbitMQ, Elasticsearch, Cassandra, Neo4j theo UI riêng.
- `v2.0.0` / Phase 24 — chỉ mở khi có breaking change thật: đổi adapter contract, đổi storage profile, hoặc redesign workflow lớn có migration path.

Commit/release mẫu cho mỗi phase:

```txt
feat(adapter): add <engine> connection and metadata explorer
test(adapter): add <engine> contract and fixture coverage
docs(roadmap): mark phase <n> release scope
