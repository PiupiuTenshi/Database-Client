...
Backlog còn lại (mỗi mục là một release tương lai): hoàn thiện docs/fixtures/UI riêng cho MongoDB/DuckDB/Oracle, runtime tunnel/proxy, cloud warehouse/lakehouse, object storage/file transfer, message queues, XLSX import/export, virtual grid, explain-plan, PNG graph export, procedure/trigger dependency graph, process/privilege manager, Marketplace publish. Xem trạng thái chi tiết bên dưới.
...
- 🟡 **SSH tunnel** cho mọi server database: profile model + transport planner đã có; còn thiếu host key policy, local port allocation, reconnect và auth runtime.
- 🟡 **Socks Proxy / HTTP Proxy**: profile model + validation đã có; còn thiếu tích hợp driver/fallback tunnel helper.
- 🟡 **Docker connector**: discovery model đã có; còn thiếu đọc container port mapping và tạo connection từ container đang chạy.
- 🟡 **JDBC bridge**: bridge model đã có; còn thiếu sidecar/worker runtime cho engine hiếm mà Node driver không ổn định.
...
- `v1.8.0` / Phase 19 — DuckDB adapter: adapter foundation đã có; release cần docs/fixtures và packaging.
