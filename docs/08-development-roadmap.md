...
## Phase 18 — Major version preparation

Mục tiêu version: `v2.0.0` khi có breaking change ở adapter contract, storage format hoặc UI workflow.

Đánh giá tại 2026-06-21: các thay đổi từ `v1.2.0`→`v1.7.0` đều **tương thích ngược** (thêm field optional `ssl`/`favorite`, thêm method adapter, thêm command/webview — không phá storage format hay contract cũ). Vì vậy **chưa tạo `v2.0.0`**; tiếp tục tăng MINOR `v1.x.0` cho tới khi có breaking change thực sự (đổi storage format profile, đổi chữ ký `DatabaseAdapter` theo cách phá vỡ, hoặc thay đổi workflow UI lớn). Khi đó mới mở milestone `v2.0.0` kèm migration path.
