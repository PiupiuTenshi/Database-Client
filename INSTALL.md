...
5. Nếu VS Code không cho cài tự động và báo lỗi như `No Servers`, extension sẽ tự thử cài bằng VS Code CLI. Nếu cả hai cách đều fail, chọn **Open VSIX**, **Reveal File**, **Copy Path** hoặc **Copy Command** để cài bằng UI **Extensions** → `...` → **Install from VSIX...** hoặc chạy lệnh đã copy.
